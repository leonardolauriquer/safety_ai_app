...
rem 'git pull' e um atalho para 'git fetch' seguido por 'git merge'.
...
if !pull_result! neq 0 (
    ...
    echo Possiveis causas: Conflitos de merge (se alteracoes locais colidirem com as do remoto),
    echo problemas de rede, ou configuracoes do Git (ex: credenciais expiradas).
    echo Se houver conflitos, o Git marcara os arquivos afetados.
...
rem
rem IMPORTANTE: Se voce reescreveu o historico do Git (ex: para remover um segredo que ja foi commitado),
rem pode ser necessario usar 'git push --force-with-lease origin main' manualmente na primeira vez.
rem Use '--force-with-lease' com cautela, pois ele sobrescreve o historico remoto,
rem mas e mais seguro que '--force' puro, pois verifica se ninguem mais fez push enquanto voce reescrevia.
