    ...
    with st.container():
        ...
        st.markdown(
            ...
            <div class="page-header">
                ...
                <h1>Consulta de Multas NR 28</h1>
            </div>
            <div class="page-subtitle">
                Cálculo de penalidades com base na NR 28, considerando número de funcionários
        ...
        if fines_processor.df_itens is not None:
            ...
            st.markdown(
                '<div class="info-hint"><b>Aviso:</b> Dados de itens de infração não disponíveis.</div>',
                unsafe_allow_html=True,
            )

        if st.button("Calcular Multa Total", type="primary", use_container_width=True):
            if not selected_item_codes:
                st.markdown(
                    ...
                    "<b>Atenção:</b> Selecione pelo menos um item irregular.</div>",
                    unsafe_allow_html=True,
                )
                return
            ...
            if total_base > 0:
                ...
                st.markdown(
                    ...
                    <div class="result-card">
                        <div class="result-title">Valor Base da Multa</div>
                        <div class="result-code" style="font-size:1.4em;">
                            R$ {format_currency_br(total_base)}
                        </div>
                        <div class="result-meta">
                            Base de cálculo: {"Reincidência" if has_recidivism else "Máximo regulamentar"}
                        </div>
                        <div class="result-detail" style="color:#FFD700;border-top-color:rgba(255,215,0,0.2);">
                            <b>⚠️ Estimativa CLT (Art. 201):</b><br>
                ...
                st.markdown(
                    ...
                    f'{_get_material_icon_html("list")} Detalhamento das Infrações</div>',
                    unsafe_allow_html=True,
                )
                for d in details:
                        ...
                        <div class="result-card" style="padding:10px 14px;margin-bottom:6px;">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
                                <div>
                                    ...
                                    <div class="result-title" style="margin-top:3px;font-size:0.85em;">{item_desc}</div>
                                    <div class="result-meta" style="margin-top:2px;">{base}</div>
                                </div>
                                <div style="text-align:right;flex-shrink:0;">
                                    <div class="result-code" style="font-size:1em;">R$ {format_currency_br(valor)}</div>
                                </div>
                            </div>
                        </div>
                        """,
                ...
                st.markdown(
                    '<div class="empty-state">Nenhum valor calculado para os parâmetros selecionados.</div>',
                    unsafe_allow_html=True,
                )
