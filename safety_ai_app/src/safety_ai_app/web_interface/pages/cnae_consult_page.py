...
_PILL_CSS = """
<style>
div[data-testid="stRadio"][data-key="cnae_level"] > div {
    gap: 6px !important;
    flex-wrap: wrap !important;
}
div[data-testid="stRadio"][data-key="cnae_level"] label {
...
div[data-testid="stRadio"][data-key="cnae_level"] label:has(input:checked) {
    background: rgba(129,140,248,0.14) !important;
    border-color: rgba(129,140,248,0.45) !important;
    color: #818CF8 !important;
    font-weight: 600 !important;
}
div[data-testid="stRadio"][data-key="cnae_level"] input { display: none !important; }
</style>
"""
...
def _build_breadcrumb(entry: Dict[str, Any]) -> str:
    ...
    secao = (
        entry.get("secao")
        or entry.get("divisao", {}).get("secao")
        or entry.get("grupo", {}).get("divisao", {}).get("secao")
        or entry.get("classe", {}).get("grupo", {}).get("divisao", {}).get("secao")
    )
    if secao:
    ...
    divisao = (
        entry.get("divisao")
        or entry.get("grupo", {}).get("divisao")
        or entry.get("classe", {}).get("grupo", {}).get("divisao")
    )
    if divisao:
...
def cnae_consult_page() -> None:
    ...
    with st.container():
        ...
        st.markdown(
            ...
            <div class="page-subtitle">
                Classificação Nacional de Atividades Econômicas (IBGE) — com grau de risco NR.
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with c2:
            search = st.text_input(
                "Buscar",
                placeholder="Ex: 0111-3/01, 01.11-3, A, Agricultura...",
                key="cnae_search",
                label_visibility="collapsed",
            ).strip()
        ...
        if not search:
            st.markdown(
                ...
                <div class="info-hint">
                    ...
                    <b>Dica:</b> Digite um código CNAE ou palavra-chave e selecione o nível hierárquico.
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    if not results:
        st.markdown(
            ...
            <div class="empty-state">
                {search_icon}
                <div>Nenhum CNAE encontrado para
                <b>"{html_module.escape(search)}"</b> no nível <b>{html_module.escape(level)}</b></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    if total_found > CNAE_LIMIT:
        st.markdown(
            ...
            <div class="stats-line">
                Exibindo <b>{CNAE_LIMIT}</b> de <b>{total_found}</b> resultados — refine para ver mais específicos.
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f"""
            <div class="stats-line">
                <b>{total_found}</b> resultado{"s" if total_found != 1 else ""} encontrado{"s" if total_found != 1 else ""}
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    for entry in displayed:
        ...
        if activities:
            items = ''.join([
                f'<div class="act-item">· {html_module.escape(str(a))}</div>'
                for a in activities[:4]
            ])
            more = (
                f'<div class="act-more">+ {len(activities) - 4} atividade(s)</div>'
                if len(activities) > 4 else ''
            )
